...
test("box selection respects camera, reverse drags, visibility candidates and far zoom", () => {
  const { boxSelectPlanetNodes } = require("./planet-hit-testing.ts");
  const nodes = [node("dir", "directory"), node("file", "file", "md"), node("outside", "file", "md")];
  const positions = new Map([["dir", {x: 10, y: 10}], ["file", {x: 20, y: 20}], ["outside", {x: 90, y: 90}]]);
  const camera = {x: 100, y: -20, k: 2};
  const start = {x: 145, y: 25}, end = {x: 115, y: -5};
  assert.deepEqual(boxSelectPlanetNodes(nodes, positions, camera, start, end, "normal"), ["dir", "file"]);
  assert.deepEqual(boxSelectPlanetNodes(nodes, positions, camera, end, start, "far"), ["dir"]);
  assert.deepEqual(boxSelectPlanetNodes(nodes.slice(1), positions, camera, start, end, "near"), ["file"]);
  assert.deepEqual(boxSelectPlanetNodes(nodes, positions, camera, {x: 0,y: 0}, {x: 1,y: 1}, "near"), []);
});
